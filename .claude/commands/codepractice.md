...
Arguments: $ARGUMENTS
(pass a file path, directory path, or `--check` to report only without fixing)

## Step 1 — Determine scope

- Parse $ARGUMENTS. If empty, use `.` (current working directory).
- If `--check` is present in $ARGUMENTS, report violations but do not modify files.
- Identify all code files in scope. Supported extensions: `.py`, `.js`, `.ts`, `.java`, `.cs`, `.go`, `.rb`, `.swift`, `.kt`, `.cpp`, `.c`, `.rs`.

## Step 2 — Read each file and identify violations

**Rule 1 — camelCase variables**
- Every variable declaration, parameter name, local binding, and field/property must use camelCase.
- camelCase: starts with a lowercase letter, each subsequent word starts uppercase. No underscores, no ALL_CAPS (unless the language mandates it for true compile-time constants — note but do not change those).
...
- Every private method must have a name starting with `_`.
...
After applying fixes, print: `Fixed <N> violation(s) in <M> file(s).`

## Step 5 — Cross-file renames (if any)

If any renamed symbol is referenced in other files, list them and ask:
